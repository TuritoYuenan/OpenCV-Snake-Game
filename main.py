...
class snakeCVclass:
    ...
    def update(self, mainIMG, headCurrent):
        ...
        if self.gameOver:
            ...
            distance = math.hypot(currentX - previousX, currentY - previousY)                   #distance = sqrt([currentX - previousX]^2 + [currentY - previousY]^2)
            ...
            if (randomX - self.foodWidth // 2 < currentX < randomX + self.foodWidth // 2        # Check if the currentX and currentY coordinates of the snake has touched the boundaries of the food
            ...
            if self.point:
                for i, point in enumerate(self.point):
                    if i != 0:
                        cv2.line(mainIMG, self.point[i-1], self.point[i], (0, 0, 255), 20)      #cv2.line(image, start_point, end_point, color, thickness) 
...
while True:
    ...
    if restart_game:
        game = snakeCVclass("battery.png")                                                  # Create a new instance of the game object
        restart_game = False
